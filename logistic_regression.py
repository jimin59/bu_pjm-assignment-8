...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        plt.plot(xx[0], -(beta0 + beta1 * xx[0]) / beta2, color='black', linestyle='--', label='Decision Boundary')
        ...
        slope = -beta1 / beta2 if beta2 != 0 else np.inf
        intercept = -beta0 / beta2
        ...
        plt.text(x_min + 0.1, y_max - 0.5, equation_text, fontsize=16, color="black", ha='left', bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.3'))
        plt.text(x_min + 0.1, y_max - 1.0, margin_text, fontsize=16, color="black", ha='left', bbox=dict(facecolor='white', edgecolor='black', boxstyle='round,pad=0.3'))
